...
%
% If called from within a sport module (CFB, MCB, NFL), this function will 
% clear all current state, and initialize paths for that module in addition
% to the base SportsEngine platform.
%
% If called from within an application (SkunkWorks, DynamiteRankings),
% this function will clear all current state, and initialize paths for that
...
% environment, this function will clear all current state and reinitialize 
% for the new context.
%
% Global variables
...
% called.
%
% There is no harm in calling this function more than is technically required, 
% other than run time.
%
function setEnvironment
	...
	if ~isempty(i_se)
		% === Do not assume a / is included, but add it back on
		sports_engine_path = sprintf('%s/', curr_path(1:i_se+length('SportsEngine')-1));
		sports_engine_initialized = true;
	else
		fprintf('==========\n');
		fprintf('FILE: Shared/setEnvironment.m');
	...
	if ~isempty(module_path)
		if ~isempty(i_ss)
			application_path = sprintf('%sSkunkWorks/', module_path);
			addpath(sprintf('%sSkunkWorks/', module_path));
			addpath(sprintf('%sSkunkWorks/Models/', module_path));
		elseif ~isempty(i_dr)
			application_path = sprintf('%DynamiteRankings/', module_path);
			addpath(sprintf('%DynamiteRankings/', module_path));
		end
	else
		% === If we're not in a module then we shouldn't have a named folder
